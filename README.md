...
1. [GPS 갱신 주기 최적화 (Adaptive Interval)](#1-gps-갱신-주기-최적화-adaptive-interval)
...
## 1. GPS 갱신 주기 최적화 (Adaptive Interval)

### 핵심 아이디어

> GPS를 항상 켜두면 배터리가 빨리 닳는다. 목적지에서 멀리 있을 때, 또는 움직이지 않을 때는 위치를 자주 확인할 필요가 없다.

시스템은 매번 "다음에 언제 GPS를 다시 켤까?"를 자동으로 계산합니다. 세 가지 요소를 조합해서 최종 갱신 주기를 결정합니다.

---

### 1단계 — 기본 주기 (Base Interval)

목적지(또는 가장 가까운 Geofence 경계)까지의 거리에 따라 기본 주기를 결정합니다.

| 목적지까지 거리 | GPS 갱신 주기 | GPS 모드     |
|----------------|--------------|-------------|
| 200m 이내      | 3초           | HIGH (정밀)  |
| 200 ~ 500m     | 10초          | BALANCED     |
| 500m ~ 1km     | 30초          | BALANCED     |
| 1km 이상       | 60초          | LOW (절전)   |
...
### 2단계 — 활동 인식 (Activity Recognition)

최근 위치 이력에서 평균 속도를 계산해 현재 이동 상태를 추정하고, 배율을 곱합니다.

| 평균 속도       | 활동 분류 | 배율  | 의미                          |
|----------------|----------|------|-------------------------------|
| 0.5 m/s 미만   | 정지      | × 3.0 | 이동 없음 → 갱신 주기를 늘림   |
| 0.5 ~ 8 m/s    | 도보      | × 1.0 | 기본 주기 유지                 |
| 8 m/s 이상     | 차량      | × 0.3 | 빠른 이동 → 갱신 주기를 줄임   |

→ 차에 타면 빠르게 이동하므로 GPS를 더 자주 확인합니다.

---

### 3단계 — 유의미한 위치 변화 감지 (Significant Location Change)

오래된 위치 기록과 현재 위치를 비교해 실제로 이동했는지 판단합니다.

| 조건                         | SLC 배율 | 효과                      |
|-----------------------------|---------|---------------------------|
| 500m 이상 이동 (유의미한 변화) | × 1.0   | 계속 적극적으로 추적         |
| 500m 미만 이동 + 정지 상태    | × 2.0   | 배터리 절약 (갱신 주기 증가) |
...
```
다음 갱신 주기 = min(base × 활동배율 × SLC배율, 300초)
```
...
| 상황                             | 계산                    | 결과   |
|---------------------------------|------------------------|--------|
| 1km 이상, 차량, 유의미한 이동     | 60 × 0.3 × 1.0 = 18초  | 18초   |
| 1km 이상, 정지 중, 이동 없음      | 60 × 3.0 × 2.0 = 360초 → 최대 300초 | 300초 |
| 300m 이내, 도보                  | 10 × 1.0 × 1.0 = 10초  | 10초   |
...
| gps_mode   | next_interval_sec | 의미                          |
|------------|-------------------|-------------------------------|
| `HIGH`     | 3초               | 목적지 200m 이내 — 정밀 추적  |
| `BALANCED` | 10 ~ 30초         | 일반 이동 중                  |
| `LOW`      | 30 ~ 300초        | 정지 중 또는 멀리 있음        |
